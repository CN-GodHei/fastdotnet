<template>
	<div class="fdrole-container">
				...
				<el-table-column label="操作" width="280" fixed="right" align="center">
					<template #default="scope">
						...
						<el-button :disabled="scope.row.IsDefault || scope.row.Code === 'SUPER_ADMIN'" icon="ele-Delete"
							size="small" text type="danger" @click="handleDelete(scope.row)">删除</el-button>
					</template>
				</el-table-column>
			</el-table>
		...
		<!-- 分配权限对话框 -->
		<el-dialog v-model="state.permissionDialog.visible" draggable :close-on-click-modal="false" width="800px"
			height="600px" style="--el-dialog-padding-primary: 0;">
			<template #header>
				<div>
...
const openAssignPermissionsDialog = async (row: APIModel.FdRoleDto) => {
	try {
		...
		// 获取菜单按钮数据
		const menuBtnData = (await FdMenuApi.getFdMenuGetMenuBtn({ Belong: 0, RoleId: row.Id as string })) as unknown as APIModel.MenuBtnRe[];
