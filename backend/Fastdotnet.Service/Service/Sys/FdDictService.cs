...
namespace Fastdotnet.Service.Service.Sys
{
    ...
    public class FdDictService : IFdDictService
    {
        ...
        {
            // 如果传入数据为空，直接返回
            if (dictTypeAndDataList == null || dictTypeAndDataList.Count == 0)
            {
                return;
            }

            // 过滤掉 fdDictType 或 fdDictData 为空的数据项
            var validDataList = dictTypeAndDataList
                .Where(item => item.fdDictType != null && item.fdDictData != null && item.fdDictData.Count > 0)
                .ToList();
            ...
            foreach (var typeCode in validDataList.Select(x => x.fdDictType.Code).Distinct())
            {
