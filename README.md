...
- ✅ **SQL 执行日志**：基于 SqlSugar AOP 拦截，自动记录每次执行的 SQL（含参数值）到 `log_sql_execution` 按天分表，支持慢 SQL 排查和链路追踪。（通过 `SqlSugar.EnableSqlExecutionLogging` 配置开关控制）
...
- ✅ **全局异常处理**：统一的异常捕获，BusinessException 返回 422，不可控异常落库 + 返回 500
- ✅ **业务操作日志**：完整的操作审计追踪（操作人、IP、请求头/体、状态码、耗时），按天自动分表
- ✅ **异常日志入库**：不可控异常自动写入 `log_exception` 表，含异常类型、堆栈、请求路径
- ✅ **调试日志**：开发调试用业务打点日志，支持 Key 标识和 RequestId 链路追踪
